...
const [api, app, styles, shell, navigation, shared, security] = await Promise.all([
...
assert.match(navigation, /requiresProperty/);
assert.match(shell, /roleLabel/);
assert.match(shared, /return allowed \? <>{children}<\/>/);
